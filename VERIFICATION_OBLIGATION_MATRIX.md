...
| framework role contracts | `framework` declarations in the five `framework_*.cdc` files carry `requires=`/`permits=`; `cdc_boot.py` enforces role completeness, uniqueness, orphan closure, exactly-one executable link, link existence, and role-primitive compatibility through `expect framework <key> complete` and `expect frameworks closed` | native/C parity for the framework-contract checker under the bootloader deletion gate |
| task-loop composition | `framework_loop.cdc` executes two sense/act/integrate cycles over one shared state object in a single `run`/`interpret` invocation, with second-cycle expectations (carried phase 0.492228, cumulative belief 1.333333) reachable only through first-cycle mutations; surface/council/evolve modes exercise gate, record, recall, refine, key, decide, and enact over the same source and the adopted coordinate equals the recorded key | single-invocation executor fusing all modes over one live state object; unbounded cycle iteration |
